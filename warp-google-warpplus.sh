...
# 配置 WARP+ Unlimited 密钥 (交互式)
configure_warp_plus() {
    echo -e "\n${CYAN}══════════════ WARP+ Unlimited 密钥配置 ══════════════${NC}\n"
    
    # 检查 WARP 是否已安装
    if ! command -v warp-cli &>/dev/null; then
        echo -e "${RED}错误: WARP 客户端未安装！${NC}"
        echo -e "${YELLOW}请先选择选项 1 安装 WARP${NC}\n"
        return 1
    fi
    
    # 显示当前账户信息
    echo -e "${YELLOW}【当前账户信息】${NC}"
    ACCOUNT_INFO=$(warp-cli --accept-tos registration show 2>/dev/null || warp-cli account 2>/dev/null || warp-cli registration show 2>/dev/null)
    if [ -n "$ACCOUNT_INFO" ]; then
        echo -e "$ACCOUNT_INFO"
    else
        echo -e "${RED}未注册或无法获取账户信息${NC}"
    fi
    echo ""
    
    # 提示用户
    echo -e "${MAGENTA}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${MAGENTA}║  WARP+ Unlimited 密钥说明:                                  ║${NC}"
    echo -e "${MAGENTA}║                                                             ║${NC}"
    echo -e "${MAGENTA}║  1. 密钥格式: xxxxxxxx-xxxxxxxx-xxxxxxxx (36位含连字符)      ║${NC}"
    echo -e "${MAGENTA}║  2. 可从 1.1.1.1 App 获取密钥，或使用第三方获取工具          ║${NC}"
    echo -e "${MAGENTA}║  3. 每个密钥可绑定最多 5 台设备                              ║${NC}"
    echo -e "${MAGENTA}║  4. 配置成功后将获得无限流量                                 ║${NC}"
    echo -e "${MAGENTA}╚════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    
    # 询问是否继续
    echo -e "${YELLOW}请选择操作:${NC}"
    echo -e "  ${GREEN}1.${NC} 输入新的 WARP+ 密钥"
    echo -e "  ${GREEN}2.${NC} 查看当前密钥/账户状态"
    echo -e "  ${GREEN}3.${NC} 重置为免费账户"
    echo -e "  ${GREEN}0.${NC} 返回主菜单"
    echo ""
    
    read -p "请输入选项 [0-3]: " plus_choice
...
# 输入 WARP+ 密钥
input_warp_plus_key() {
    echo -e "\n${CYAN}请输入您的 WARP+ Unlimited 密钥:${NC}"
    echo -e "${YELLOW}(格式: xxxxxxxx-xxxxxxxx-xxxxxxxx，输入 q 取消)${NC}\n"
    
    while true; do
        read -p "密钥: " LICENSE_KEY
        
        # 检查是否取消
        if [ "$LICENSE_KEY" = "q" ] || [ "$LICENSE_KEY" = "Q" ]; then
            echo -e "\n${YELLOW}已取消操作${NC}\n"
            return 0
        fi
        ...
        # 验证密钥格式 (基本格式检查: 包含连字符，长度约36字符)
        if [ -z "$LICENSE_KEY" ]; then
            echo -e "${RED}错误: 密钥不能为空，请重新输入${NC}\n"
            continue
        fi
        
        # 检查格式 (xxxxxxxx-xxxxxxxx-xxxxxxxx)
        if ! echo "$LICENSE_KEY" | grep -qE '^[a-zA-Z0-9]{8}-[a-zA-Z0-9]{8}-[a-zA-Z0-9]{8}$'; then
            echo -e "${RED}错误: 密钥格式不正确${NC}"
            echo -e "${YELLOW}正确格式: xxxxxxxx-xxxxxxxx-xxxxxxxx (8位-8位-8位)${NC}\n"
            
            read -p "是否仍要尝试使用此密钥? (y/n): " force_use
        ...
        # 确认密钥
        echo -e "\n${YELLOW}您输入的密钥:${NC} ${GREEN}$LICENSE_KEY${NC}"
        read -p "确认使用此密钥? (y/n): " confirm
        
        if [ "$confirm" = "y" ] || [ "$confirm" = "Y" ]; then
            break
        else
            echo -e "${YELLOW}请重新输入密钥${NC}\n"
        fi
    done
    
    # 应用密钥
    echo -e "\n${CYAN}正在配置 WARP+ 密钥...${NC}"
    ...
    # 方式1: 新版命令
    if warp-cli --accept-tos registration license "$LICENSE_KEY" 2>&1; then
        SET_RESULT="success"
    # 方式2: 旧版命令
    elif warp-cli --accept-tos set-license "$LICENSE_KEY" 2>&1; then
        SET_RESULT="success"
    # 方式3: 另一种旧版命令
    elif warp-cli registration license "$LICENSE_KEY" 2>&1; then
        SET_RESULT="success"
    # 方式4: 
    elif warp-cli set-license "$LICENSE_KEY" 2>&1; then
        SET_RESULT="success"
    # 方式5: 使用 license 子命令
    elif warp-cli license "$LICENSE_KEY" 2>&1; then
        SET_RESULT="success"
    fi
    ...
    # 获取账户信息
    ACCOUNT_TYPE=$(warp-cli --accept-tos registration show 2>/dev/null | grep -i "type\|account\|plan" || warp-cli account 2>/dev/null | grep -i "type\|account\|plan")
    
    if echo "$ACCOUNT_TYPE" | grep -qi "plus\|unlimited\|warp+"; then
        echo -e "${GREEN}╔════════════════════════════════════════════════════╗${NC}"
        echo -e "${GREEN}║       🎉 WARP+ Unlimited 配置成功！ 🎉              ║${NC}"
        echo -e "${GREEN}╚════════════════════════════════════════════════════╝${NC}"
        echo -e "\n${GREEN}您已成功升级到 WARP+ Unlimited！${NC}"
    else
        echo -e "${YELLOW}密钥已提交，请检查账户状态:${NC}"
    fi
    ...
    # 重启透明代理
    if [ -f /usr/local/bin/warp-google ]; then
        echo -e "\n${CYAN}正在重启透明代理...${NC}"
        /usr/local/bin/warp-google restart 2>/dev/null
    fi
...
show_warp_account_info() {
    ...
    if ! command -v warp-cli &>/dev/null; then
        echo -e "${RED}WARP 客户端未安装${NC}\n"
        return 1
    fi
    
    # 获取注册信息
    echo -e "${YELLOW}【注册信息】${NC}"
    warp-cli --accept-tos registration show 2>/dev/null || warp-cli registration show 2>/dev/null || warp-cli account 2>/dev/null || echo "无法获取注册信息"
    echo ""
    
    # 获取连接状态
    echo -e "${YELLOW}【连接状态】${NC}"
    warp-cli --accept-tos status 2>/dev/null || warp-cli status 2>/dev/null || echo "无法获取状态"
    echo ""
    
    # 获取设置信息
    echo -e "${YELLOW}【当前设置】${NC}"
    warp-cli --accept-tos settings 2>/dev/null || warp-cli settings 2>/dev/null || echo "无法获取设置"
    echo ""
    
    # 检测账户类型
    ACCOUNT_OUTPUT=$(warp-cli --accept-tos registration show 2>/dev/null || warp-cli account 2>/dev/null)
    
    if echo "$ACCOUNT_OUTPUT" | grep -qi "plus\|unlimited\|warp+"; then
        echo -e "${GREEN}账户类型: WARP+ Unlimited ✓${NC}"
    elif echo "$ACCOUNT_OUTPUT" | grep -qi "free\|limited"; then
        echo -e "${YELLOW}账户类型: WARP Free (免费版)${NC}"
    else
        echo -e "${YELLOW}账户类型: 未知${NC}"
    fi
...
# 重置为免费账户
reset_warp_account() {
    echo -e "\n${YELLOW}警告: 此操作将重置 WARP 账户到免费版本${NC}"
    ...
    # 删除注册信息
    warp-cli --accept-tos registration delete 2>/dev/null || warp-cli delete 2>/dev/null || true
    sleep 1
    
    # 重新注册
    echo -e "${CYAN}正在重新注册...${NC}"
    warp-cli --accept-tos registration new 2>/dev/null || warp-cli --accept-tos register 2>/dev/null || true
    sleep 2
...
case "$1" in
    ...
    plus)
        ...
        read -p "密钥: " key
        warp-cli disconnect 2>/dev/null
        warp-cli registration license "$key" 2>/dev/null || warp-cli set-license "$key" 2>/dev/null
        sleep 2
        warp-cli connect 2>/dev/null
        echo "已尝试应用密钥，请检查账户状态"
        warp-cli registration show 2>/dev/null || warp-cli account 2>/dev/null
        ;;
    account)
        echo "WARP 账户信息:"
        warp-cli registration show 2>/dev/null || warp-cli account 2>/dev/null
...
do_install() {
    ...
    echo -e "\n${MAGENTA}提示: 如需配置 WARP+ Unlimited，请选择菜单选项 4${NC}"
    echo -e "\n管理命令: ${CYAN}warp {status|start|stop|restart|test|ip|plus|account|uninstall}${NC}\n"
}
...
show_menu() {
    ...
    echo -e "  ${GREEN}4.${NC} ${MAGENTA}配置 WARP+ Unlimited 密钥${NC}"
    echo -e "  ${GREEN}5.${NC} 启动 WARP"
    echo -e "  ${GREEN}6.${NC} 停止 WARP"
